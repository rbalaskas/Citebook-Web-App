...
import { like_post,if_liked,delete_post,my_post,get_picture_url,file_flag,edit_post} from '../actions';
...
const Post=(props)=> {
  ...
  const [picture_url, setPicture_url] = useState("http://localhost:8000/posts_pdf/profile_pic.jpg");
 ...
 // handleClick = () => this.setLiked((prevState) => ({ active: !prevState.active }))
  let url_file= `http://localhost:8000/api/get_file/${props.post_prop.id}`;
  console.log(url_file);
 // const [file, setFile] = useState("${url_file}");
 // console.log(props.post_prop.file_path);
      ...
      >
        ...
        <Modal.Content>
          <h3 style={{"textAlign":"center"}}>
            Are you sure you want to delete your post: "{props.post_prop.title}"?
          </h3>
        </Modal.Content>
        <Modal.Actions>
          <Button basic color='green' inverted onClick={() => setOpen2(false)}>
            <Icon name='remove' /> No
          </Button>
...
{props.post_prop.users_id === props.userId &&
    ...
    <Modal.Content >    
         ...
         <Form>
          <Form.Field>
            ...
            <label>Abstract:</label>
                      ...
                      <Header as='h4' style={{"marginBottom":"0px"}}floated='left'>
                        Upload pdf: 
                      </Header> 
                  <a>{filename} </a>
                  <br/>
       ...
       <Modal.Actions>
         ...
         <Button icon className="d" type='submit' onClick={submit_post}>
            ...
            <div className="info">
              <span><Link to={`/citebook/profile/${props.post_prop.users_id}`}>{props.post_prop.fname} {props.post_prop.lname}</Link> </span>
              <br/>
              <span>Published: {date}</span><span>,  Authors: {props.post_prop.authors}</span>
              <br/>  {!props.post_prop.google_scholar && <span>#{props.post_prop.keywords}</span>}
              {props.post_prop.google_scholar && <a  href={props.post_prop.keywords}>{props.post_prop.keywords}</a>}
            </div>
          </div>
          ...
          <Button
            ...
            icon='heart'
            size='mini'
            label={{ basic: true, pointing: 'left', content: `${props.post_prop.likes}` }}
            onClick={handleClick}
          />
      ...
      closeIcon
      open={open}
      trigger={ <Button  disabled={pdf_flag} icon className="paper"  size='small'>
